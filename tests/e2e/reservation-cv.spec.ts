...
test("サンクスページはorderIdクエリを予約番号として表示できる", async ({ page }) => {
  ...
  await expect(page.locator("#botchan_order_id")).toHaveText("GYM-20260619-9999");
  await expect(page.locator("#botchan_order_id")).toHaveCSS("display", "none");
});
...
test("LPから無料体験予約を完了し、サンクスページでオーダーIDを確認できる", async ({ page }) => {
  ...
  await expect(page.locator("#botchan_order_id")).toHaveText(/^GYM-\d{8}-\d{4}$/);
});
